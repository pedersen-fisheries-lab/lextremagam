...
peak_detect_parallel <- function(sim_data, logname){ 
  ...
  sim_dat_detect_results_list <- foreach (i = unique(sim_data$sample_id), #iterating for each ID number
                                          .packages=c('dplyr', 'R.utils','pracma', "mgcv", "lextremagam", "marginaleffects"))%dopar% {
...
#merges list output into a dataframe, where each iteration output is a row
    ...
    #quantify local extrema
      #fit gam
      tic()
      sim_gam <- mgcv::gam(y ~ s(x, bs = "ad", m = 3), 
                           method = "REML", 
                           data = test_data)
      ...
      #Finding lextrema
      lextrema_summary <- quantify_lextrema(mod = sim_gam, 
                                           var = "x", 
                                           step_size = 0.0005, 
...
tic()
#sim_e1_results <- peak_detect_parallel(sim_data = sim_dat_e1, logname = "test_log")
