...
#Export data to parquet
setwd("~/Documents/Pedersen2023/Peaks/lextremagam")
write_parquet(x = sim_dat_e2, "data-raw/sim/data/sim_dat_e3.parquet")
...
sim_dat_e4 <- sim_dat_e4%>%
  ...
  dplyr::mutate(y = y_true + rnorm(n = n(),mean = 0, sd = error))%>%
  dplyr::ungroup()%>%
  dplyr::filter((func=="var-scale" & (param <=0.66 &param>0.19 |param>0.91))|
                  (func=="var-curvature"& (param>0.19 & param<=0.81 | param>0.91))|
                  func == "var-location")

# setwd("~/Documents/Pedersen2023/Peaks/lextremagam/data-raw/sim/data")
# write_parquet(x = sim_dat_e1, "data-raw/sim/data/sim_dat_e4.parquet")
...
ggplot(data=filter(sim_dat_e4, rep==1, dup==1, func=="var-location"))+
  geom_point(aes(x=x, y=y, colour=as.factor(param)))+
  geom_line(aes(x=x, y=y_true, colour=as.factor(param)))+
  facet_wrap(~as.factor(param))
```
