...
hs1195w1_lextr <- lextremagam::quantify_lextrema(mod = hs1195w1_gam, 
                                                 ...
                                                 step_size = 0.02, 
                                                 conf_level = 0.95, 
                                                 deriv_method = "gratia", frequentist = FALSE)

hs1195w1_lextr_plot <- plot_lextrema(quant_segments = hs1195w1_lextr, plot_deriv = TRUE)
hs1195w1_lextr$segment_summary

hs_plot <- draw(hs1195w1_gam, residuals = TRUE, rug = FALSE)+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_start[2]), 
             colour = "firebrick", linetype = 2)+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_end[2]),
             colour = "firebrick", linetype = 2)+
  labs(x = "time since disturbance (years)", title = "")+
  theme_bw()

hs1195w1_lextr_plot$model_plot+
  geom_point(aes(x = HalpernSpies1995_w1$t_disturbance, y =HalpernSpies1995_w1$heterogeneity), colour = "#6699cc")+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_start[3]), 
             colour = "firebrick", linetype = 2)+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_end[3]),
             colour = "firebrick", linetype = 2)+
  labs(x = "time since disturbance (years)", title = "")
hs1195w1_lextr_plot$deriv_plot+labs(x = "time since disturbance (years)", y = "First derivative of partial effect")+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_start[3]), 
             colour = "firebrick", linetype = 2)+
  geom_vline(aes(xintercept = hs1195w1_lextr$segment_summary$t_disturbance_end[3]),
             colour = "firebrick", linetype = 2)
...
wk_plot <- draw(wk1988_gam, residuals = TRUE, rug = FALSE)+
  labs(x = "ln(% somc)", title = "")+
  scale_y_continuous(limits = c(-6, 2))+
  theme_bw()

wk1988_lextr_plot$deriv_plot+
  labs(x = "ln(% somc)", title = "", y = "first derivative of partial effect")+
...
shrimp_dat_n0 <- shrimp_dat%>%
  filter(Pandalus.borealis != 0)

plot(shrimp_dat$Pandalus.borealis~shrimp_dat$depth)

plot(log(shrimp_dat$Pandalus.borealis+0.01)~shrimp_dat$depth)

#super skewed
# shrimp_gam <- gam(Pandalus.borealis ~s(depth)+s(lat_dec)+s(year_surv, bs="re")+s(NAFO, bs="re"), data=shrimp_dat, method="REML")
# 
# shrimp_gam <- gam(Pandalus.borealis ~s(depth, bs = "ad", k = 20, m = 3)+s(lat_dec)+s(year_surv, bs="re")+s(NAFO, bs="re"), data=shrimp_dat, method="REML")

shrimp_gam <- gam(Pandalus.borealis ~s(depth)+s(lat_dec)+s(year_surv, bs="re")+s(NAFO, bs="re"), data=shrimp_dat, method="REML", family = tw())

# shrimp_dat$sqrt_depth <- sqrt(shrimp_dat$depth)
# shrimp_gam <- gam(Pandalus.borealis ~s(sqrt_depth)+s(lat_dec)+s(year_surv, bs="re")+s(NAFO, bs="re"), data=shrimp_dat, method="REML", family = tw())
# 
# shrimp_gam <- gam(Pandalus.borealis ~s(depth)+s(lat_dec)+s(year_surv, bs="re")+s(NAFO, bs="re"), data=shrimp_dat_n0, method="REML", family = Gamma)

appraise(shrimp_gam)
draw(shrimp_gam, residuals = FALSE, rug = FALSE)
...
# shrimp_lextr <- lextremagam::quantify_lextrema_multivar(mod = shrimp_gam, var = "sqrt_depth", 
#                                                         smooth = "s(sqrt_depth)", conf_level = 0.95, 
#                                                         deriv_method = "gratia", frequentist = FALSE, step_size = 0.020)

shrimp_lextr_freq <- lextremagam::quantify_lextrema_multivar(mod = shrimp_gam, var = "depth", 
                                                        smooth = "s(depth)", conf_level = 0.95, 
                                                        deriv_method = "gratia", frequentist = TRUE, step_size = 0.020)

lextremagam::plot_lextrema(quant_segments = shrimp_lextr, plot_deriv = FALSE, show_segs = "local_max",show_segs_deriv = FALSE)

shrimp_lextr$segment_summary
shrimp_lextr_freq$segment_summary

dfo_fig <- draw(shrimp_gam, residuals = FALSE, select = "s(depth)", rug = FALSE)
dfo_fig+
  geom_vline(aes(xintercept = shrimp_lextr$segment_summary$depth_start[2]), colour = "firebrick", linetype = 2)+
  geom_vline(aes(xintercept = shrimp_lextr$segment_summary$depth_end[2]), colour = "firebrick", linetype = 2)+
