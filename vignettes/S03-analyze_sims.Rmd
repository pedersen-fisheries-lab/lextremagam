...
get_coverage <- function(lextr_me, lextr_gr, func_type){
  coverage_me <- lextr_me%>%
    filter(func %in% func_type)%>%
    rowwise()%>%
    mutate(capture = case_when(
      feature %in% c("local_max", "boundary_max", "notrend") ~ (param>=x_start & param<=x_end),
...
get_weirdos <- function(peak_test, lextr_me, lextr_gr, func_type){
...
e1_lextrema_results_me$feature[which(is.na(e1_lextrema_results_me$lag) &
                                          is.na(e1_lextrema_results_me$lead)&
                                          e1_lextrema_results_me$x_end >0.999999&
                                          e1_lextrema_results_me$x_start<0.0000001&
                                          e1_lextrema_results_me$slope_sign ==0)] <- "notrend"

e1_lextrema_results_gratia <- read_parquet("data-raw/sim/results/lextrema_gratia_results_e1.parquet")
e1_lextrema_results_gratia$feature[which(is.na(e1_lextrema_results_gratia$feature))] <- "notrend"
